...
public class EffectsManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        BonusRunnersParent.OnPileOfCoin += RewardPileOfCoin;
    }
    ...
    public void RewardPileOfCoin(int coinReward)
    {
        ...
        for (int i = 0; i < pileOfCoinParent.childCount; i++)
        {
            ...
            child.GetComponent<RectTransform>().DOAnchorPos(new Vector2(500f, 850f), 1f).SetDelay(delay).SetEase(Ease.OutBack);

            if (i == pileOfCoinParent.childCount - 1)
                child.DOScale(0f, 0.3f).SetDelay(delay + 1f).SetEase(Ease.OutBack).OnComplete(() =>
                {
